...
func main()  {
	...
	//frequencyForWord := map[string]int{} // 返回一个指针，指向一个map实体，相当于make(map[string]int)
	frequencyForWord := make(map[string]int)
	for _,filename := range commandLineFile(os.Args[1:]){
		updateFrequency(filename, frequencyForWord)
	}
	reportByWords(frequencyForWord)
	return
}

func commandLineFile(files []string) []string  {
...
func updateFrequency(filename string, frequencyForWord map[string]int)  {
	var file *os.File
	var err error
	if file, err = os.Open(filename); err != nil {
		log.Panicln("fail to open the file:", err)
		return
	}
	defer file.Close()
...
func reportByWords(frequencyForWord map[string]int) {
	...
	fmt.Printf("%d, %s, %s",gap, "", "frequency")
